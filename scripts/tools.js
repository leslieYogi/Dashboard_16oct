...
function Dashboard() {
    ...
    self.tabletPalette = "DashboardPaletteTablet";
    self.mobilePalette = "DashboardPaletteMobile";
    ...
    self.loadPage = function (activeTab) {
        ...
        xhr = $.ajax({
            dataType: "html",
            url: ["views/", url, ".html?ae7b73167afb443587a276572b338dfc"].join(""),
    ...
    self.initRangeSelector = function() {
        ...
        rangeContainer.dxRangeSelector({
            ...
            chart: {
                series: [{ type: 'line', color: '#4c92ce', opacity: 0.8, argumentField: 'SaleDate', valueField: 'Sales' }]
            },
            dataSource: [],
    ...
    self.initGrid = function(gridCustomOptions) {
        ...
        return {
            ...
            load: function(loadOptions, category) {
                ...
                gridXhr = Dashboard.loadData(loadOptions, function (data) {
    ...
    $("a").each(function () {
        if (!this.onclick && this.getAttribute("target") != "_blank") {
            this.onclick = function () {
                return Dashboard.loadPage(this.hash.substring(1));
            };
        }
    });
    $(window).bind("resize", Dashboard.setScreenSize);
    Dashboard.setScreenSize();
    Dashboard.loadPage(window.location.hash.substring(1) || "Index");
  
    $("#currentDate").text((Globalize.formatDate(Dashboard.getDate(), { date: "medium" })).toUpperCase());
